...
def _load_from_macos_keychain() -> dict | None:
    ...
    Uses the built-in `security` CLI to avoid any third-party dependency.
    Returns the parsed JSON payload, or None if the item is missing or
    ...
    try:
        ...
        # FileNotFoundError covers the unlikely case of `security` not being on PATH.
        pass
    return None
...
def _load_from_windows_credential_manager() -> dict | None:
    ...
    Requires the `CredentialManager` PowerShell module (ships with recent
    Windows versions). Returns the parsed JSON payload, or None on failure.

    Note: this path is untested on a real Windows machine — feedback from
    Windows users is welcome.
    """
    ps_script = (
...
def _load_from_credentials_file() -> dict | None:
    ...
    This is the storage path used on Linux and WSL, and serves as a
    universal fallback for any platform where the native secret store
    lookup fails.
    """
...
def load_credentials() -> OAuthCredentials:
    ...
    # Fallback to the credentials file for Linux, WSL, or when the
    # platform-native store lookup returned nothing.
    if data is None:
